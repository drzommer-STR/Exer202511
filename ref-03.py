...
with st.sidebar:
    # API Key 입력
    st.markdown('<h2 style="color: #1f77b4;">⚙️ 설정</h2>', unsafe_allow_html=True)
    ...
    if not api_key:
        st.warning("⚠️ OpenAI API Key를 입력해주세요.")
        st.stop()
                ...
                try:
                    ...
                    if not all_docs:
                        ...
                        if st.session_state.vectorstore is None:
                            ...
                            if vectorstore is None:
                                if last_error:
                                    raise Exception(f"벡터 스토어 생성에 실패했습니다: {last_error}")
                                else:
                                    raise Exception("벡터 스토어 생성에 실패했습니다. API Key를 확인해주세요.")
                            ...
                            for i in range(0, len(chunks), batch_size):
                                ...
                                try:
                                    ...
                                    st.warning(f"일부 청크 추가 중 오류 발생: {str(e)}")
                                    continue
                        ...
                        if st.session_state.vectorstore is not None:
                            ...
                            raise Exception("벡터 스토어가 생성되지 않았습니다.")
                    ...
                    if "429" in error_msg or "insufficient_quota" in error_msg.lower() or "quota" in error_msg.lower():
                        st.error("⚠️ **OpenAI API 할당량 초과**")
                        st.error("현재 사용 중인 API Key의 할당량이 소진되었습니다.")
                        st.info("""
                        **해결 방법:**
                        1. OpenAI 계정의 사용량과 결제 정보를 확인하세요
                        2. 새로운 API Key를 발급받아 사용하세요
                        3. API Key의 할당량 제한을 확인하고 필요시 업그레이드하세요
                        ...
                        st.error("⚠️ **OpenAI API 인증 오류**")
                        st.error("API Key가 유효하지 않습니다.")
                        st.info("""
                        **해결 방법:**
                        1. API Key가 올바르게 입력되었는지 확인하세요
                        2. 새로운 API Key를 발급받아 사용하세요
                        3. https://platform.openai.com/api-keys 에서 API Key를 확인하세요
                        """)
                    elif "rate limit" in error_msg.lower():
                        st.error("⚠️ **API 호출 한도 초과**")
                        st.error("API 호출 빈도 제한에 걸렸습니다.")
                        st.info("잠시 후 다시 시도해주세요.")
                    else:
                        st.error(f"파일 처리 중 오류가 발생했습니다: {error_msg}")
    ...
    if st.session_state.retriever is None:
        ...
        with st.spinner("답변을 생성 중입니다..."):
            try:
                ...
                # OpenAI API 오류 처리
                if "429" in error_msg or "insufficient_quota" in error_msg.lower() or "quota" in error_msg.lower():
                    error_response = "⚠️ **OpenAI API 할당량 초과**\n\n현재 사용 중인 API Key의 할당량이 소진되었습니다.\n\n**해결 방법:**\n1. OpenAI 계정의 사용량과 결제 정보를 확인하세요\n2. 새로운 API Key를 발급받아 사용하세요\n3. API Key의 할당량 제한을 확인하고 필요시 업그레이드하세요"
                elif "401" in error_msg or "authentication" in error_msg.lower() or "invalid" in error_msg.lower():
                    error_response = "⚠️ **OpenAI API 인증 오류**\n\nAPI Key가 유효하지 않습니다.\n\n**해결 방법:**\n1. API Key가 올바르게 입력되었는지 확인하세요\n2. 새로운 API Key를 발급받아 사용하세요"
                elif "rate limit" in error_msg.lower():
                    error_response = "⚠️ **API 호출 한도 초과**\n\nAPI 호출 빈도 제한에 걸렸습니다. 잠시 후 다시 시도해주세요."
                else:
                    error_response = f"오류가 발생했습니다: {error_msg}"
